        ...
        <div class="index-content" id="index-content">
            <IndexContent ref="icRef">
                <template v-slot:default>
                    <!--首页文章信息-->
                    <div v-if="$store.state.articles !== undefined">
                        <!-- 文章信息 -->
                        <div class="index-content-item" v-for="(item) in $store.state.articles" :key="item.id">
                            <nuxt-link :to="'/articles/'+item.id" class="article-img-cover">
                                <el-image lazy class="hvr-grow" fit="cover" :src="item.cover" v-if="item.cover !== ''">
                                ...
                                <el-image lazy class="hvr-grow" fit="cover" :src="$store.state.randomPicUrl + '?' + item.id" v-else>
                                    <div slot="placeholder" class="image-slot" style="width: 100%; height: 100%;">
                                        <img src="/loading.gif" style="width: 100%;height: 100%;">
                                    </div>
                                    <div slot="error" class="image-slot" style="width: 100%; height: 100%;">
                            ...
                            <div class="article-info">
                                <nuxt-link :to="'/articles/'+item.id" class="article-title">{{ item.title }}
                                </nuxt-link>
                                    ...
                                    <div class="article-wrap">
                                        ...
                                        <div class="ato">
                                            ...
                                            <nuxt-link :to="{name:'labels-name',params:{name:tag}}"
                                                       v-for="(tag, index) in item.labelTags" :key="index">{{ tag }}
                                            </nuxt-link>
                                            <span class="separator"> |</span>
                                        </div>
                                        <div class="ato">
                                            ...
                                            <nuxt-link to="#">{{ item.viewCount }}</nuxt-link>
                                            <span> 阅读量</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="article-summary" style="cursor: pointer"
                                     @click="$router.push({path: '/articles/'+item.id})">
...
<script>
import IndexBgHeader from "@/components/IndexBgHeader.vue";
import IndexContent from "@/components/IndexContent.vue";
import {getPageNeededData} from "@/assets/utils/utils";
import {showH1WithSign, showSocial, SUCCESS} from "@/assets/api/api";

export default {
    name: "IndexPage",
    components: {IndexBgHeader, IndexContent: IndexContent},
    methods: {
        // 切换文章页面
    ...
    // 异步执行，请求数据
    async asyncData(ctx) {
        await getPageNeededData(ctx)    // 获取必要信息
        if (ctx.store.state.h1WithSign.h1 === undefined || ctx.store.state.h1WithSign.sign === undefined) {
            const res = await showH1WithSign()
            if (res.code === SUCCESS) {
                ctx.store.commit("setH1WithSign", {
                    h1: res.data['web_site_h1'],
                    sign: res.data['web_site_sign']
                })
            } else {
                ctx.store.commit("setH1WithSign", {
                    h1: 'Hello,world!',
        ...
        if (ctx.store.state.socialMedias === undefined) {
            const res = await showSocial()
            if (res.code === SUCCESS) {
                ctx.store.commit("setSocialMedias", res.data)
            }
        }
    },
};
