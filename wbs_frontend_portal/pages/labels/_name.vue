        ...
        <div class="index-content" id="index-content">
            <IndexContent ref="icRef">
                <template v-slot:default>
                    <div>
                        <!-- 文章信息 -->
                        <div class="index-content-item" v-for="(item, index) in $store.state.articles" :key="index">
...
import {showArticlesByLabelName, SUCCESS} from "@/assets/api/api";
...
export default {
    ...
    async asyncData(ctx) {
        ...
        let name = encodeURI(ctx.params.name)   // URL编码一下
        const articlesByLabelNameRes = await showArticlesByLabelName(name, 1)
        if (articlesByLabelNameRes.code === SUCCESS) { // 覆盖设置
            ctx.store.commit("setArticles", articlesByLabelNameRes.data)
        }
    },
    mounted() {
        // 设置标题内容
        document.getElementById("pages-title").innerHTML = "标签：" + this.$route.params.name
    }
}
</script>
